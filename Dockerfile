...
RUN apk add --no-cache curl bash unzip python3 py3-pip aria2 ffmpeg py3-cryptography py3-pillow && \
    curl https://rclone.org/install.sh | bash && \
    pip3 install --no-cache-dir --break-system-packages yt-dlp vcsi && \
    rm -rf /var/cache/apk/*
